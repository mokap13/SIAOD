...
void main()
{
	...
	coordinates = fileManager.getCoordinates();
	fileManager.ReadFile("Regions.MIF");
	graphicManager.setBrush(0xcc, 0x0, 0xff);
	for (list<double*>::iterator i = coordinates->begin(); i != coordinates->end(); i++)
	{
		graphicManager.printPolygon(*i);
	}
	coordinates->clear();

	fileManager.ReadFile("Houses.MIF");
	coordinates = fileManager.getCoordinates();
	graphicManager.setBrush(0xff, 0xff, 0);
	for (list<double*>::iterator i = coordinates->begin(); i != coordinates->end(); i++)
	{
		graphicManager.printPolygon(*i);
	}
	...
	graphicManager.setBrush(0, 0xff, 0xff);
	for (list<double*>::iterator i = coordinates->begin(); i != coordinates->end(); i++)
	{
		graphicManager.printPolygon(*i);
	}
	...
	fileManager.ReadFile("Bounds.MIF");
	graphicManager.setPen(0, 0, 0xff,2);
	for (list<double*>::iterator i = coordinates->begin(); i != coordinates->end(); i++)
	{
		graphicManager.printLine(*i);
	}
	coordinates->clear();
